...
// Added boilerplate to work with google's import syntax

goog.provide('windmill.Sound');
goog.scope(function() {
...
windmill.Sound = {};
windmill.Sound.soundEffect = soundEffect;
...
function endSound(count) {
  windmill.Sound.soundEffect(
    900,          //frequency
    0.05,         //attack
    0.1,          //decay
    "sine",       //waveform
    .1 / (count * count),            //volume
    0.8,          //pan
    0,            //wait before playing
...
function clearSounds() {
  for (var i=0; i<CURRENT_AUDIO_TIMEOUTS.length; i++) {
    clearTimeout(CURRENT_AUDIO_TIMEOUTS[i]);
  }
}
windmill.Sound.playNothing = clearSounds;
windmill.Sound.playStartRipples = function() {
  clearSounds();
  // put them in an array so they can all be cancelled
  CURRENT_AUDIO_TIMEOUTS = [
    setTimeout(function() {startSound(1); }, 2000),
    setTimeout(function() {startSound(2); }, 6000),
    setTimeout(function() {startSound(3); }, 10000),
    setTimeout(function() {startSound(4); }, 14000),
    setTimeout(function() {startSound(5); }, 18000),
    setTimeout(function() {startSound(6); }, 22000),
    setTimeout(function() {startSound(7); }, 26000),
  ]
}

windmill.Sound.playEndRipples = function() {
  clearSounds();
  // put them in an array so they can all be cancelled
  CURRENT_AUDIO_TIMEOUTS = [
    setTimeout(function() {endSound(1); }, 600),
    setTimeout(function() {endSound(2); }, 2100),
    setTimeout(function() {endSound(3); }, 3600),
    setTimeout(function() {endSound(4); }, 5100),
    setTimeout(function() {endSound(5); }, 6600),
    setTimeout(function() {endSound(6); }, 8100),
